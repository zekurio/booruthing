...
import { Search, X, ArrowRight, Plus, Minus } from "lucide-react";
import { useState, useRef, useEffect } from "react";
import { useQuery } from "@tanstack/react-query";
import { TagMode, TagWithMode, AutocompleteResult } from "~/lib/types";
...
export default function SearchPage() {
  ...
  const { data: suggestions = [], isLoading } = useQuery({
    queryKey: ["autocomplete", search.trim()],
  ...
  const getTagModeIcon = (mode: TagMode) => {
    switch (mode) {
      ...
      case "optional":
        return <span className="text-xl font-mono font-bold">~</span>;
    }
  };

  const cycleTagMode = () => {
    const modes: TagMode[] = ["include", "exclude", "optional"];
  ...
  return (
    <div className="flex flex-col items-center min-h-screen">
      <div className="flex flex-col items-center gap-4 sm:gap-8 w-full pt-8 sm:pt-16">
        <Logo />
        <div className="w-full max-w-2xl space-y-4 px-2 sm:px-4">
          <Popover open={showSuggestions && search.trim().length >= 2} onOpenChange={setShowSuggestions}>
