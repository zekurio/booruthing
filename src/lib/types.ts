...
export const TagMode = z.enum(["include", "exclude", "optional"]);
...
export const PostsApiResponse = z.union([
  z.array(Post),
  z.string().length(0).transform(() => []), // Empty string becomes empty array
  z.null().transform(() => []), // Null becomes empty array
  z.undefined().transform(() => []), // Undefined becomes empty array
]);
